...
- 效率
- 群里可以发表情，不要发斗图
- QQ 群消息不要屏蔽，所有的作业，链接，都会发到群里
- 为什么不用 wiki 或者 github 什么的呢？
- 关于我跟同学【大声与小声】讨论问题
- 教学/创业等问题
- 学生的【学习能力】本来就很差，你怎么办
- 没有好的学习习惯
...
- 学的时候不要依赖男朋友。。。
- 女生在 IT 行业中的就职情况
- 一般来说，在人数稍多的团队中（10 人左右），同等条件下，会招女生
...
- f(x,y) = x _ x + 2 _ y
- f(3,4)
- f(a,b,c,d) = a + b _ b + 2 _ c + d / 5
- f(3,2,4,5) = 3 + 2 _ 2 + 2 _ 4 + 5 / 5 = 16
- 映射
- 从一个域的值 对应 到另一个域的值 。
- {1 => a
- 2 => a
...
* git Linus(Linux 早期)
  ...
  git init ------ git add+（需要添加的文件名）------git commit -m "备注信息"-------git push -u origin master 添加文件并且推送
...
* iframe
<!-- 可以单独嵌入一个别的网页进来  src嵌入网页地址-->
<!--注：由于安全原因，某些网页拒绝被嵌入其它页面  -->
  - inline frame
  - 必须有开始标签和结束标签
  ...
  * code
    ...
    ```
<!-- iframe可以连环套  target=_top  顶层窗口  _parent 副窗口-->   
